...
## D-011: 局送りは「物理席固定・親と風が回る」(連荘・本場あり)
...
- **What**: 東風戦 (東1〜東4) の局送りを実装 (2026-06)。設計の要点:
  ...
  - **親の連荘あり** (2026-06 に「連荘なし」から変更): `startNextRound()` が連荘を判定する。親が和了 → 連荘 (`roundIndex` 据え置き)・本場+1。子が和了 → 親交代・本場0リセット。流局は本場+1 し、親が形式テンパイ (`winningTiles(dealer.hand, dealer.melds).length>0`) なら連荘・ノーテンなら親交代 (本場は引き継ぐ)。親が北家 (東4局) から連荘せず流れた時点で `phase="round_end"` (終局・最終順位表示) で点数は動かない。`startMatch()` で 25000点×4・東1・0本場に戻る
  - **本場 (honba)**: `GameState.honba` で保持し `#deal(roundIndex, honba)` が次局へ運ぶ。和了点への加算は `calcScore({..., honba})` (ツモは支払者1人あたり +100×本場、ロンは放銃者 +300×本場)。点数移動の総和は 0 のままなので不変条件 `Σ score + riichiPot === 100000` は保たれる
