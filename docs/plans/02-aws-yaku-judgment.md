...
- AWS パターン照合 (`matchesCountSuperset`) に渡す手牌は `winning/melds.ts:effectiveHandTiles` の**実効14枚** (純手牌 + 各副露3枚分)。**カンは3枚の刻子として射影** (`toDecompMeld`) し、「counts34 の各要素 ≤4」の不変条件を保つ。yaku.json に同一牌4枚を要求する sample が無いことは検証済み
