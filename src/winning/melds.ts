...
/** 副露を分解結果用 Meld に射影する。kan 系は3枚の刻子 (pon) として扱う */
...
/** 役判定 (AWS パターン照合等) に渡す実効手牌: 純手牌 + 各副露3枚分 */
export function effectiveHandTiles(concealed: Tile[], melds: MeldLike[]): Tile[] {
  return [
    ...concealed,
    ...melds.flatMap((m) => m.tiles.slice(0, MELD_TILES_FOR_DECOMP)),
  ];
}

/** 門前判定: 副露なし、または暗槓のみなら門前 (標準ルール通り) */
